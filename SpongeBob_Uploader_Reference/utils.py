...
class _ColourFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        fmt_str = f"{colour}%(asctime)s | %(levelname)-8s | %(name)s | %(message)s{self.RESET}"
...
def setup_logging(level: int = logging.INFO) -> logging.Logger:
    ...
    Returns the root logger so callers can optionally use it directly.
    """
...
def slow_type(page: Page, selector: str, text: str,
              min_delay_ms: int = 80, max_delay_ms: int = 180) -> None:
    """
    ...
    Args:
        ...
        selector:      CSS / XPath selector for the target input element.
...
def slow_fill(page: Page, selector: str, text: str) -> None:
    """
    Use Playwright's fill() to set a field value instantly (for long text
    such as descriptions or tags), preceded by a human delay.
    """
    human_sleep(0.3, 0.8)
...
def move_episode_to_done(ep_path: Path, done_dir: Path) -> None:
    """
    Move a completed *Ep_XX* folder into the *Uploaded_Done* directory.
    If the destination already exists (e.g. from a previous partial move),
    the source folder is merged by renaming with a _duplicate suffix.
    """
    import shutil
    logger = logging.getLogger(__name__)

    destination = done_dir / ep_path.name
    if destination.exists():
        destination = done_dir / f"{ep_path.name}_duplicate"
        logger.warning(
            "Destination '%s' already exists — moving to '%s' instead.",
            done_dir / ep_path.name,
            destination,
        )
    shutil.move(str(ep_path), str(destination))
    logger.info("Moved '%s' → '%s'", ep_path.name, destination)
